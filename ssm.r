...
SEIT4L_inputs <- list(
	...
	input(name="R0", description="basic reproduction number", prior=unif(1,50), value=8.8904999), 
	input(name="D_lat", description="latent period", prior=truncnorm(mean=2,sd=1,a=0), value=1.6551303),
	input(name="D_inf", description="infectious period", prior=truncnorm(mean=2,sd=1,a=0), value=2.5233450),
	...
	input(name="alpha", description="infectious period", prior=unif(0,1), value=0.4776501),
	input(name="D_imm", description="infectious period", prior=unif(0,50), value=12.2714425),
	input(name="rho", description="reporting rate", prior=unif(0,1), value=0.6649321),
...
my_ssm_fit_ode <- my_ssm %>% SSMinR::pmcmc(iter=10000, cooling=0.99, eps_switch=100, switch=500) %>% SSMinR::pmcmc(approx="psr", n_parts=100,iter=1000, cooling=0.99, eps_switch=10, switch=50) 

my_ssm_fit_ode %>% plot_X
